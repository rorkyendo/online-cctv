<div class="card shadow-sm">
    <div class="card-header border-0 pt-6">
        <div class="card-title">
            <h3 class="fw-bold">Edit Lokasi</h3>
        </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ url('/panel/lokasi/updateLokasi/' . $data['lokasi']->id_lokasi . '/save') }}">
            @csrf
            <div class="mb-5">
                <label class="form-label required fw-semibold">Nama Lokasi</label>
                <input type="text" name="nama_lokasi" class="form-control"
                       value="{{ old('nama_lokasi', $data['lokasi']->nama_lokasi) }}" required />
            </div>
            <div class="mb-5">
                ...
                <select name="id_group" class="form-select" required>
                    ...
                    @foreach($data['grupList'] as $grup)
                        <option value="{{ $grup->id_group }}"
                                {{ $data['lokasi']->id_group == $grup->id_group ? 'selected' : '' }}>
                            {{ $grup->nama_group }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-5">
                ...
                <textarea name="deskripsi" class="form-control" rows="3">{{ old('deskripsi', $data['lokasi']->deskripsi) }}</textarea>
            </div>
